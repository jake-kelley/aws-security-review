...
| check_id | Flags | Severity | Ref |
|----------|-------|----------|-----|
| `ec2_public_snapshots` | EBS snapshot restorable by *anyone* | CRITICAL | EC2.1 |
| `ec2_public_amis` | AMI shared with all AWS accounts | HIGH | — |
| `ec2_sg_open_admin_ports` | Security group allows SSH/RDP (or all traffic) from `0.0.0.0/0` / `::/0` | HIGH (attached) / MEDIUM (dormant) | CIS 5.2/5.3 |
| `ec2_default_sg_restricts_traffic` | Default security group still has rules | MEDIUM (in use) / LOW | CIS 5.4 |
| `ec2_ebs_default_encryption` | EBS encryption-by-default off for the region | MEDIUM | CIS 2.2.1 |

Per instance (stopped instances are checked too; terminated ones skipped):

| check_id | Flags | Severity | Ref |
|----------|-------|----------|-----|
| `ec2_imdsv2_required` | IMDSv1 still answers (`HttpTokens` ≠ `required`; disabled endpoint counts as a pass) | HIGH | CIS 5.6 |
| `ec2_instance_public_ip` | Instance has a public IPv4 address | MEDIUM | EC2.9 |
| `ec2_instance_ebs_encrypted` | An attached EBS volume is unencrypted | MEDIUM | EC2.3 |
| `ec2_instance_exposed_admin_port` | Public IP **and** an attached SG opening admin ports — directly attackable (FAIL-only synthesis of the two) | HIGH | SEC05 |
...
| check_id | Flags | Severity | Ref |
|----------|-------|----------|-----|
| `lambda_function_public` | Resource policy `Allow` to principal `*` — unconditioned means anyone can invoke; condition-constrained is `COND` in the table | CRITICAL / MEDIUM | Lambda.1 |
| `lambda_url_auth` | Function URL with `AuthType: NONE` (unauthenticated HTTPS endpoint) | HIGH | — |
| `lambda_runtime_supported` | Deprecated (end-of-life) runtime — no more AWS security patches | MEDIUM | Lambda.2 |
| `lambda_env_secrets` | Environment variable **names** matching credential patterns (`PASSWORD`, `SECRET`, `TOKEN`, ...) | MEDIUM | — |
...
| check_id | Flags | Severity | Ref |
|----------|-------|----------|-----|
| `ecs_task_privileged` | A container runs `privileged: true` (host device access → escape = host root) | HIGH | ECS.4 |
| `ecs_task_host_network` | `networkMode: host` (shares host network, bypasses awsvpc isolation) | HIGH | ECS.1 |
| `ecs_task_host_pid` | `pidMode: host` (can see/signal every host process) | HIGH | ECS.3 |
| `ecs_task_readonly_rootfs` | A container's root filesystem is writable | MEDIUM | ECS.5 |
| `ecs_task_nonroot_user` | A container runs as root (or no `user` set) | MEDIUM | ECS.1 |
| `ecs_task_env_secrets` | Plaintext env var **names** look like secrets (should use `secrets`/valueFrom) | HIGH | ECS.8 |
| `ecs_task_logging` | A container has no `logConfiguration` | MEDIUM | ECS.9 |

Per service:

| check_id | Flags | Severity | Ref |
|----------|-------|----------|-----|
| `ecs_service_public_ip` | `assignPublicIp: ENABLED` (tasks get a public IP) | MEDIUM | ECS.2 |
...
| check_id | Flags | Severity | Ref |
|----------|-------|----------|-----|
| `eks_endpoint_public` | API endpoint public to `0.0.0.0/0` (FAIL). Public but CIDR-restricted is a PASS flagged `COND` for review | HIGH / MEDIUM | EKS.1 |
| `eks_version_supported` | Kubernetes version below the standard-support floor (point-in-time) | MEDIUM | EKS.2 |
| `eks_secrets_encryption` | No `encryptionConfig` for `secrets` (no KMS envelope encryption) | MEDIUM | EKS.3 |
| `eks_audit_logging` | Control-plane `audit` log type not enabled | MEDIUM | EKS.8 |
...
floor is point-in-time (mid-2026); refresh `_MIN_SUPPORTED_K8S` in `eks.py`
